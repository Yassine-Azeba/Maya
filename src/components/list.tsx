...
export default function List({userId,planeId,lines}:ListProps){
    ...
    return (
        <div className="w-full flex flex-col gap-0.5">
            {firstLine.map(line => <ListItem key={line.lineId} lineId={line.lineId} lines={lines} />)}
        </div>
    )
}
...
function ListItem({lineId,lines}:ListItemProps){
    ...
    return(
        <>
            ...
            {isChildrenOpen?childrens.map(child => <ListItem key={child.lineId} lineId={child.lineId} lines={lines} />):<></>}
        </>
    )
}